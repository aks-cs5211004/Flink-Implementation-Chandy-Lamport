...
class CkptAckRecvMsg(RcvMsg):
  ...
  def update(self, state: CoordinatorState, source: str) -> Optional[PHASE]:
    ...
    if worker.last_cp_id < self.checkpoint_id:
      worker.last_cp_id=self.checkpoint_id
    all_checkpoint_done= all(ws.last_cp_id >= state.next_cp_id for _, ws in state.workers.items())
    if all_checkpoint_done:
      logging.info(f"CHECKPOINT COMPLTED :------------> {state.next_cp_id}")
      state.next_cp_id+=1
      return PHASE.CP
    return None
...
class LastCkptAckRecvMsg(RcvMsg):
  ...
  def update(self, state: CoordinatorState, source: str) -> Optional[PHASE]:
    ...
    worker= state.workers[source]
    worker.last_checkpoint_done = True
    all_last_cp_done= all(ws.last_checkpoint_done for _,ws in state.workers.items())
    if(all_last_cp_done):
      logging.info("All workers completed their last checkpoints, transitioning to  EXIT phase")
...
class RecoveryAckRecvMsg(RcvMsg):
  ...
  def update(self, state: CoordinatorState, source: str) -> Optional[PHASE]:
    ...
    # Count how many mappers and reducers have recovered
    recovered_mappers = sum(
        ws.recovery_id == state.next_recovery_id and ws.is_mapper
        for _, ws in state.workers.items() if ws.is_mapper
    )
    recovered_reducers = sum(
        ws.recovery_id == state.next_recovery_id and not ws.is_mapper
        for _, ws in state.workers.items() if not ws.is_mapper
    )
    # logging.info(f"Recoverd Reducers: {recovered_reducers}, Recovered Mappers: {recovered_mappers}")
    if recovered_reducers==2 and recovered_mappers==0:
        # logging.info("All reducers have recovered, transitioning to RECOVER_MAPPER phase")
        return PHASE.RECOVER_MAPPER
    if recovered_mappers==2:
        logging.info(f"RECOVERY COMPLETED ..............> {self.recovery_id}")
        return PHASE.CP
    return None 
...
class DoneRecvMsg(RcvMsg):
  ...
  def update(self, state: CoordinatorState, source: str) -> Optional[PHASE]:
    ...
    if(self.msg=='not_done'):
      state.workers[source].is_done = False
    ...
    if are_all_mappers_done:
      logging.info("All Mappers Done, move to LAST CP")
...
def msg_factory(message: Message) -> RcvMsg:
  if message.msg_type == MT.HEARTBEAT:
    ...
    return DoneRecvMsg(message.kwargs['msg'])
...
class SendThread(threading.Thread):
  ...
  def cp_phase(self):
    ...
    # logging.info(f"{self.id} sending CP commands to WORKERS")
    for _, ws in self.state.workers.items():
      if ws.is_mapper: 
  ...
  def recover_phase(self, is_mapper: bool) -> None:
    # logging.info(f"Current Phase in State {self.state.phase}")
    # logging.info(self.state.phase)
    ...
    if is_mapper:
      # logging.info(f"{self.id} sending recover commands to MAPPERS")
      for _, ws in self.state.workers.items():
        if ws.is_mapper:
          RecoveryMsg(checkpoint_id, self.state.next_recovery_id).send(self.state.sock, ws.addr)
    else:
      self.state.next_recovery_id+=1
      # logging.info(f"{self.id} sending recover commands to REDUCERS")
      for _, ws in self.state.workers.items():
        if not ws.is_mapper:
          RecoveryMsg(checkpoint_id, self.state.next_recovery_id).send(self.state.sock, ws.addr)
  ...
  def last_cp_phase(self):
    ...
    logging.info(f"{self.id} sending last CP commands to WORKERS")
    for _, ws in self.state.workers.items():
      if ws.is_mapper:
